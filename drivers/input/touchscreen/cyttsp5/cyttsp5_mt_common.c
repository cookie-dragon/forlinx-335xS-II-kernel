...
static void cyttsp5_get_touch_axis(struct cyttsp5_mt_data *md, int *axis,
		int size, int max, u8 *xy_data, int bofs) {
	int nbyte;
	int next;
...
static void cyttsp5_get_touch_hdr(struct cyttsp5_mt_data *md,
	...
	for (hdr = CY_TCH_TIME; hdr < CY_TCH_NUM_HDR; hdr++) {
		if (!si->tch_hdr[hdr].report)
			continue;
		cyttsp5_get_touch_axis(md, &touch->hdr[hdr], si->tch_hdr[hdr].size,
				si->tch_hdr[hdr].max, xy_mode + si->tch_hdr[hdr].ofs,
				si->tch_hdr[hdr].bofs);
		dev_vdbg(dev, "%s: get %s=%04X(%d)\n", __func__,
				cyttsp5_tch_hdr_string[hdr], touch->hdr[hdr], touch->hdr[hdr]);
	}

	dev_dbg(dev, "%s: time=%X tch_num=%d lo=%d noise=%d counter=%d\n",
			__func__, touch->hdr[CY_TCH_TIME], touch->hdr[CY_TCH_NUM],
			touch->hdr[CY_TCH_LO], touch->hdr[CY_TCH_NOISE],
...
static void cyttsp5_get_touch_record(struct cyttsp5_mt_data *md,
	...
	for (abs = CY_TCH_X; abs < CY_TCH_NUM_ABS; abs++) {
		if (!si->tch_abs[abs].report)
			continue;
		cyttsp5_get_touch_axis(md, &touch->abs[abs], si->tch_abs[abs].size,
				si->tch_abs[abs].max, xy_data + si->tch_abs[abs].ofs,
				si->tch_abs[abs].bofs);
		dev_vdbg(dev, "%s: get %s=%04X(%d)\n", __func__,
				cyttsp5_tch_abs_string[abs], touch->abs[abs], touch->abs[abs]);
	}
}
...
static void cyttsp5_mt_process_touch(struct cyttsp5_mt_data *md,
	...
	dev_vdbg(dev, "%s: flip=%s inv-x=%s inv-y=%s x=%04X(%d) y=%04X(%d)\n",
			...
			touch->abs[CY_TCH_X], touch->abs[CY_TCH_X], touch->abs[CY_TCH_Y],
			touch->abs[CY_TCH_Y]);
}
...
#ifdef  CONFIG_ANDROID
	if(event==CY_TCH_X) {
		input_report_abs(md->input, ABS_MT_POSITION_X, 800 - value);
	} else if(event==CY_TCH_Y) {
#ifdef  CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_7INCH
		input_report_abs(md->input, ABS_MT_POSITION_Y, 480 - value);
#else
		input_report_abs(md->input, ABS_MT_POSITION_Y, 480 - value);
#endif
//	} else if(event==ABS_MT_WIDTH_MAJOR) {
//		input_report_abs(md->input, ABS_MT_WIDTH_MAJOR, 1);
	} else if(event==CY_TCH_MAJ) {
		input_report_abs(md->input, ABS_MT_TOUCH_MAJOR, value);
		input_report_abs(md->input, ABS_MT_WIDTH_MAJOR, value);
	} else if(event==CY_TCH_T) {
		input_report_abs(md->input,ABS_MT_TRACKING_ID,value);
...
#else
	if (event == CY_TCH_X) {
		input_report_abs(md->input, ABS_X, 800 - value);
	} else if (event == CY_TCH_Y) {
#ifdef  CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_7INCH
		input_report_abs(md->input, ABS_Y, 480 - value);
#else
		input_report_abs(md->input, ABS_Y, 480 - value);
#endif
		input_report_abs(md->input, ABS_Z, 1);
	} else if (event == ABS_PRESSURE) {
		input_report_abs(md->input, ABS_PRESSURE, 1);
	} else if (event == BTN_TOUCH) {
		input_report_key(md->input, BTN_TOUCH, 1);
	}
#endif
...
static void cyttsp5_get_mt_touches(struct cyttsp5_mt_data *md,
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		for (j = 0; j < CY_NUM_ABS_OST; j++) {

			if (!si->tch_abs[j].report)
				continue;

//			if (j == CY_TCH_MAJ) {
//				if (tch->abs[CY_TCH_E] == CY_EV_MOVE || tch->abs[CY_TCH_E]
//						== CY_EV_TOUCHDOWN) {
//					cyttsp5_report_event(md, CY_ABS_X_OST + j,
//							tch->abs[CY_TCH_X + j]);
//				} else {
//					cyttsp5_report_event(md, CY_ABS_X_OST + j, 0);
//				}
//			} else
				cyttsp5_report_event(md, CY_ABS_X_OST + j,
						tch->abs[CY_TCH_X + j]);
		}
		cyttsp5_report_event(md, BTN_TOUCH, 1);
#ifdef  CONFIG_ANDROID
#else
		cyttsp5_report_event(md, ABS_PRESSURE, 1);
#endif
		...
		cyttsp5_get_mt_touches_pr_tch: dev_dbg(dev,
				"%s: t=%d x=%d y=%d z=%d M=%d m=%d o=%d e=%d obj=%d tip=%d\n",
				__func__, t, tch->abs[CY_TCH_X], tch->abs[CY_TCH_Y],
				tch->abs[CY_TCH_P], tch->abs[CY_TCH_MAJ], tch->abs[CY_TCH_MIN],
				tch->abs[CY_TCH_OR], tch->abs[CY_TCH_E], tch->abs[CY_TCH_O],
				tch->abs[CY_TCH_TIP]);
	}

	if (md->mt_function.final_sync)
		md->mt_function.final_sync(md->input, si->tch_abs[CY_TCH_T].max,
				mt_sync_count, ids);
...
static int cyttsp5_xy_worker(struct cyttsp5_mt_data *md) {
	...
	cyttsp5_xy_worker_exit: return rc;
}
...
static int cyttsp5_setup_input_device(struct device *dev) {
	...
	for (i = 0; i < NUM_SIGNALS(md->pdata->frmwrk); i++) {
		...
		if(i==CY_ABS_P_OST||i==CY_ABS_W_OST||i==CY_ABS_MIN_OST||
				i==CY_ABS_OR_OST||i==CY_ABS_TOOL_OST||i==CY_ABS_D_OST)
		continue;/* only select am335 used*/
		if (signal != CY_IGNORE_VALUE) {
			...
			//printk("signal=%d,min=%d,max=%d,value1=%d,value2=%d\n",signal,min,max,MT_PARAM_FUZZ(md, i),MT_PARAM_FLAT(md, i));
			dev_dbg(dev, "%s: register signal=%02X min=%d max=%d\n", __func__,
					signal, min, max);
		}
	}
	set_bit(ABS_MT_WIDTH_MAJOR,md->input->evbit);
	...
	input_set_abs_params(md->input, ABS_TOOL_WIDTH, 0, 15, 0, 0);
#endif
...
int cyttsp5_mt_probe(struct device *dev) {
	...
	error_init_input: input_free_device(md->input);
	error_alloc_failed: error_no_pdata: dev_err(dev, "%s failed.\n", __func__);
	return rc;
}
