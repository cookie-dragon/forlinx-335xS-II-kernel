...
{
	int i;
	u32 mac_lo0, mac_lo1 , mac_hi; 

        mac_lo0 = omap_ctrl_readl(TI81XX_CONTROL_MAC_ID0_LO);
        mac_hi = omap_ctrl_readl(TI81XX_CONTROL_MAC_ID0_HI);
	mac_lo1 = omap_ctrl_readl(TI81XX_CONTROL_MAC_ID1_LO);
	...
	seq_printf(m, "Serial\t\t: %04x%08x%04x\n",
		   mac_lo0,mac_hi,mac_lo1 );
